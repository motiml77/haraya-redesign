...
export default function AdminBooksPage() {
 ...
 {isAdmin && (
 <div className="bg-[#E8DCC4] p-4 border border-[#D6C8A8]">
 <div className="flex gap-3 items-end">
 <div className="flex-1">
 <label className="block text-sm font-bold text-[#6B5D4F] mb-1">ספר חדש</label>
 <input type="text" value={newBookTitle} onChange={(e) => setNewBookTitle(e.target.value)}
 className="w-full p-2.5 border border-[#D6C8A8] bg-[#F1E6D2] focus:border-[#B14F1C] outline-none" placeholder="שם הספר..."
 onKeyDown={(e) => e.key === 'Enter' && createBook()} disabled={creatingBook} />
 </div>
 <button onClick={createBook} disabled={creatingBook}
 className="px-4 py-2.5 bg-[#B14F1C] text-white hover:bg-[#7A2525] transition-colors font-bold flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
 ...
 {loadingBooks && (
 <div className="flex items-center justify-center py-8 gap-2 text-[#6B5D4F]">
 ...
 <div key={book.id} className={`bg-[#E8DCC4] border border-[#D6C8A8] overflow-hidden ${book.isHidden ? 'opacity-50' : ''}`}>
 <div onClick={() => editingId !== `book-${book.id}` && toggleBook(book.id)}
 className="w-full flex justify-between items-center p-5 hover:bg-[#F1E6D2] transition-colors text-right cursor-pointer">
 ...
 className="p-0.5 text-[#D6C8A8] hover:text-[#B14F1C] disabled:opacity-20 disabled:cursor-default transition-colors"
 title="הזז למעלה"
 >
 <ArrowUp size={14} />
 </button>
 ...
 <BookOpen size={24} className="text-[#B14F1C]" />
 ...
 className="text-lg font-bold text-[#1F1A14] bg-[#E8DCC4] border border-[#B14F1C] px-2 py-0.5 outline-none focus:border-[#B14F1C]/30"
 autoFocus />
 ) : (
 <h3 className="text-lg font-bold text-[#1F1A14]">{book.title}</h3>
 )}
 {editingId !== `book-${book.id}` && (
 <button onClick={(e) => { e.stopPropagation(); startRename(`book-${book.id}`, book.title); }}
 className="p-1 text-[#D6C8A8] hover:text-[#B14F1C] transition-colors" title="שנה שם">
 <Pencil size={14} />
 </button>
 )}
 </div>
 ...
 className={`p-1.5 transition-colors ${book.isHidden ? 'text-[#D6C8A8] hover:text-[#1F1A14]' : 'text-[#1F1A14] hover:text-[#6B5D4F]'}`}
 ...
 className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50">
 ...
 : <ChevronDown size={20} className={`text-[#6B5D4F] transition-transform ${expandedBook === book.id ? 'rotate-180' : ''}`} />
 }
 </div>
 </div>

 {expandedBook === book.id && !loadingBookDetails && (
 <div className="border-t border-[#D6C8A8] p-5 bg-[#F1E6D2] space-y-2">
 {/* Section tree */}
 ...
 <p className="text-xs text-[#6B5D4F] text-center py-4">אין תוכן בספר זה</p>
 )}
 ...
 className="flex-1 p-2 border border-[#D6C8A8] bg-[#E8DCC4] text-sm" placeholder="שם החלק החדש..."
 ...
 className="px-3 py-2 bg-[#6B5A4E] text-white text-xs font-bold disabled:opacity-50">
 {creatingSection ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />}
 </button>
 <button onClick={() => setAddingToParent(null)} className="px-2 py-2 text-[#6B5D4F] hover:text-red-500 text-xs">✕</button>
 </div>
 ) : (
 <button onClick={() => { setAddingToParent({ bookId: book.id, parentId: null }); setNewSectionTitle(''); }}
 className="flex items-center gap-1.5 text-sm text-[#6B5D4F] hover:text-[#B14F1C] font-bold mt-3 transition-colors">
 ...
 <p className="text-center text-[#6B5D4F] py-12">אין ספרים עדיין. {isAdmin ? 'צור ספר חדש למעלה.' : ''}</p>
 )}
 </div>
 </div>
 );
...
function SectionItem({ section, index, siblingCount, bookId, isAdmin, expandedSections, onToggle, onDelete,
 ...
 const depthColors = ['bg-[#E8DCC4]', 'bg-[#F1E6D2]', 'bg-[#F5F0EA]', 'bg-[#E8DCC4]'];
 const bgColor = depthColors[Math.min(section.depth, depthColors.length - 1)];

 return (
 <div className={`${bgColor} border border-[#D6C8A8] overflow-hidden ${section.isHidden ? 'opacity-50' : ''}`}>
 ...
 <ChevronDown size={14} className={`text-[#6B5D4F] transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
 </button>
 ...
 className="text-sm font-bold text-[#1F1A14] bg-[#E8DCC4] border border-[#B14F1C] px-2 py-0.5 outline-none focus:border-[#B14F1C]/30 flex-1 min-w-0"
 autoFocus />
 </div>
 ) : (
 <Link href={`/admin/sections/${section.id}`}
 className="flex-1 flex items-center gap-2 p-2 min-w-0 cursor-pointer hover:bg-black/5 transition-colors">
 {section.hasContent && (
 <span className={`w-2 h-2 rounded-full shrink-0 ${section.isEdited ? 'bg-green-500' : 'bg-red-400'}`}></span>
 )}
 <span className="text-sm font-bold text-[#1F1A14] truncate">{section.title}</span>
 {hasChildren && (
 <span className="text-xs text-[#6B5D4F] bg-[#E8DCC4] px-1.5 py-0.5 rounded-full shrink-0">{section.children.length}</span>
 )}
 </Link>
 )}
 ...
 className={`p-1.5 transition-colors ${section.isHidden ? 'text-[#D6C8A8] hover:text-[#1F1A14]' : 'text-[#1F1A14] hover:text-[#6B5D4F]'}`}
 ...
 className="p-1.5 text-[#6B5D4F] hover:text-[#B14F1C] transition-colors" title="שנה שם">
 <Pencil size={14} />
 </button>
 <button onClick={() => onStartAdd(section.id)}
 className="p-1.5 text-[#6B5D4F] hover:text-[#6B5A4E] transition-colors" title="הוסף תת-חלק">
 <FolderPlus size={14} />
 </button>
 {isAdmin && (
 ...
 <div className="border-t border-[#D6C8A8] p-2 space-y-1.5">
 ...
 {/* Add child form */}
 {isAddingHere && (
 <div className="border-t border-[#D6C8A8] p-2">
